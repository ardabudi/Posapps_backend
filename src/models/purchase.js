...
module.exports = {
    ...
    getHistory: (startDate, endDate) => {
        return new Promise((resolve, reject) => {
            con.query(`SELECT SUM(totalPayment) AS total, DATE_FORMAT(date_added,'%Y-%m-%d') as date FROM purchase where date_added >= '${startDate}' and date_added <= DATE_ADD('${endDate}', INTERVAL 1 DAY) GROUP BY DATE(date_added)`, (error, result) => {
                if (error) reject(new Error(error))
                console.log(result)
                resolve(result)
            })
    ...
    detailPurchase: (idBuyer) => {
        return new Promise((resolve, reject) => {
            con.query(`SELECT products.name as name, pd.stock as Qty, pd.price as Price, idBuyer as id FROM purchase_detail pd
                LEFT JOIN products on pd.productId = products.id
